...
namespace PinePie.SimpleJoystick
{
    ...
    public class JoystickController : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
    {
        ...
        public void ShowAtScreen(Vector2 screenPos, Camera cam)
        {
            joystickBase.gameObject.SetActive(true);

            RectTransformUtility.ScreenPointToLocalPointInRectangle(
                (RectTransform)joystickBase.parent, screenPos, cam, out var basePos);

            joystickBase.anchoredPosition = basePos;
            handle.anchoredPosition = Vector2.zero;
            inputDirection = Vector2.zero;
            dragStartedInside = true;
        }

        public void DragByScreen(Vector2 screenPos, Camera cam)
        {
            RectTransformUtility.ScreenPointToLocalPointInRectangle(
                joystickBase, screenPos, cam, out var localPoint);

            var clamped = Vector2.ClampMagnitude(localPoint, joystickRange);
            handle.anchoredPosition = clamped;

            var raw = clamped / joystickRange;
            inputDirection = (raw.magnitude < deadZone) ? Vector2.zero : raw;

            if (directionSnaps > 1 && inputDirection != Vector2.zero)
            {
                float angle = Vector2.SignedAngle(Vector2.left, inputDirection);
                float snap = 360f / directionSnaps;
                float snapped = Mathf.Round(angle / snap) * snap;
                inputDirection = Quaternion.Euler(0, 0, snapped) * Vector2.left;
                handle.anchoredPosition = inputDirection * joystickRange;
            }
